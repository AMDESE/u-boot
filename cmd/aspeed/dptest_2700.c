...
static int
do_ast_dptest(struct cmd_tbl *cmdtp, int flags, int argc, char *const argv[])
{
	char received = 0;
	char execute_test = 0;
	int flag = 0, i;
...
uchar Adjust_CR_EQ_Train(int TrainPat, uchar ADJStatus)
{
	...
	do {
		...
		/* Set the pre-emphsis & swing value */
		value = ((readl(DP_TX_PHY_CFG) & ~(0xff000000)) | val);
		writel(value, DP_TX_PHY_CFG);
		DBG(DBG_A_SUB, "6.A_C_E Set Phy config %x\n", val);
	...
	if (clear_auto_test) {
		...
		if (auto_test_phy) {
			...
			if (preemphasis0 == 0x3 || preemphasis1 == 0x3) {
				...
				Preemphasis_Level = DP_PREEMP_1;
				DBG(DBG_ERR, "!!Pre-emp-type P_1 !!\n");
			} else if (preemphasis0 == 0x0 || preemphasis1 == 0x0) {
				Preemphasis_Level = DP_PREEMP_0;
				DBG(DBG_ERR, "!!Pre-emp-type P_0 !!\n");
			}
...
//  DP_TX_PHY_CFG: PHY_Cfg are used here by condition
...
char DPPHYTX_Show_Cfg(void)
{
	...
	PHY_Cfg		= DP_PHY_INIT_CFG | (DP_Rate | Preemphasis_Level | Swing_Level);
...
U_BOOT_CMD(dptest, 2, 0, do_ast_dptest,
	   "ASPEED Display Port phy test",
	   "ASPEED DP test v.0.0.1");
