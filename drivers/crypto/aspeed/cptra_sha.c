...
static int cptra_sha_probe(struct udevice *dev)
{
	...
	cs->scu_regs = (void *)ofnode_get_addr(node);
	if (cs->scu_regs == (void *)FDT_ADDR_T_NONE) {
		debug("cannot map SCU registers\n");
		return -ENODEV;
	}

	if (!(readl(cs->scu_regs + SCU1_CPTRA) & SCU1_CPTRA_RDY_FOR_RT)) {
		debug("unavailable Caliptra\n");
		return -ENODEV;
	}
