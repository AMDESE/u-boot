...
size_t aspeed_memmove_dma_op(void *dest, const void *src, size_t count)
{
	...
	if ((u64)dest % 4 != 0 || (u64)src % 4 != 0)
		return 0;

	if ((u64)src >= ASPEED_FMC_CS0_BASE &&
	    (u64)src < (ASPEED_FMC_CS0_BASE + ASPEED_FMC_CS0_SIZE) &&
	    (u64)dest >= ASPEED_DRAM_BASE) {
		if ((u64)dest >= ASPEED_DRAM_BASE)
			writel(0x4, (void *)DRAM_HI_ADDR);

		writel((u32)((u64)dest - ASPEED_DRAM_BASE),
		       (void *)DMA_RAM_ADDR);
		writel((u32)((u64)src - ASPEED_FMC_CS0_BASE),
		       (void *)DMA_FLASH_ADDR);
